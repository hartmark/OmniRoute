 ...
 *
 * Kept as its own row-count gate for callers/tests that want it directly; the
 * cleanup scheduler below layers this in front of the reclaimable-bytes gate
 * (getReclaimableBytes/vacuumIfWorthwhile) instead of calling this directly,
 * since row count alone can still trigger a VACUUM that frees very little
 * disk space (a routine cleanup that freed 2-6 rows re-triggered a full
 * VACUUM on every restart regardless -- see vacuumIfWorthwhile below).
 */
  ...
  setTimeout(async () => {
    try {
      ...
      if (totalDeleted > 0) {
        ...
        // Row count is a cheap first filter; the reclaimable-bytes check
        // below is the precise gate on whether VACUUM's rewrite cost is
        // actually worth paying (see vacuumIfWorthwhile's doc comment).
        if (shouldVacuumAfterCleanup(totalDeleted)) {
